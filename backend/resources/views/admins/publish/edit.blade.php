...
@section('content')
    ...
    <div class="card">
        <form method="post" id="publish-form" action="{{ route('admin.publish.insert') }}">
        {{ csrf_field() }}
        <div class="card-body">
                <div class="form-group row">
                   <label class="col-sm-2 col-form-label">タイトル名</label>
                   <div class="col-sm-8">
                       <input name="title" type="text" class="form-control" placeholder="2022年07月の従業員向け販売" value="{{ old('title') }}">
                ...
                <div class="form-group row">
                    ...
                    <div class="ml-auto">
                        <br><br><br><br>
                         @include('admins.components.buttons.ok', ['url' => route('admin.publish.insert')])
                    </div>
        </div>
        </form>
    </div>
...
<script>
    function createOptionElements(num, parentId, type) {
      var pYear = '{{ old('exhibit_year') }}';
      var sYear = '{{ old('sales_start_year') }}';
      var eYear = '{{ old('end_of_sale_year') }}';
      var pMonth = '{{ old('exhibit_month') }}';
      var sMonth = '{{ old('sales_start_month') }}';
      var eMonth = '{{ old('end_of_sale_month') }}';
      var pDay = '{{ old('exhibit_day') }}';
      var sDay = '{{ old('sales_start_day') }}';
      var eDay = '{{ old('end_of_sale_day') }}';
      var pHour = '{{ old('exhibit_hour') }}';
      var sHour = '{{ old('sales_start_hour') }}';
      var eHour = '{{ old('end_of_sale_hour') }}';
      var pMinute = '{{ old('exhibit_minute') }}';
      var sMinute = '{{ old('sales_start_minute') }}';
      var eMinute = '{{ old('end_of_sale_minute') }}';
